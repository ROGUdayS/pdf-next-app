...
export default function DashboardLayout({
  ...
  const sidebarItems = [
    ...
    { name: 'My PDFs', href: '/dashboard/pdfs', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-full px-4 sm:px-6">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <span className="ml-4 text-xl font-bold text-gray-900">PDF Culture</span>
            </div>
            <UserProfileMenu />
          </div>
        </div>
      ...
      <div 
        className={`fixed left-0 top-16 h-full bg-white shadow-sm transition-all duration-300 
          ${isMobile ? (isSidebarOpen ? 'w-64 translate-x-0' : '-translate-x-full w-64') : 'w-16'} 
          md:translate-x-0`}
      >
      ...
      {/* Main Content */}
      <div className={`pt-16 ${isMobile ? (isSidebarOpen ? 'pl-64' : 'pl-0') : 'pl-16'} transition-all duration-300`}>
        <main className="p-6">
          {children}
        </main>
      </div>
