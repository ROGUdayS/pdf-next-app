...
const AvatarWithUpload = ({ 
  ...
  return (
    <div 
      className={`relative ${sizeClasses} rounded-full overflow-hidden bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center ring-2 ring-white shadow-sm ${!isGoogleUser && showUploadHover ? 'cursor-pointer' : ''}`}
    ...
    >
      {hasValidPhoto && user?.photoURL ? (
          ...
          {shouldShowOverlay() && (
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-200">
              <svg 
                ...
                className={`${size === "small" ? "h-4 w-4" : "h-5 w-5"} text-white`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
        ...
        <>
          <span className={`${size === "small" ? "text-sm" : "text-lg"} font-semibold text-white`}>
      ...
      {isUploading && (
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className={`${size === "small" ? "w-5 h-5" : "w-6 h-6"} border-2 border-white border-t-transparent rounded-full animate-spin`}></div>
        </div>
      )}
    </div>
  );
          ...
          <div className="px-4 py-3 border-b border-gray-100">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex flex-col">
                ...
                {uploadStatus && (
                  <span className={`text-xs mt-1 ${imageError ? 'text-red-500' : 'text-blue-500'}`}>
                    {uploadStatus}
                  </span>
                )}
              </div>
